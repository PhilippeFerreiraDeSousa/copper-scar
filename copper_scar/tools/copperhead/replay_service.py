...
const view=new URLSearchParams(location.search).get('view')==='legacy'?'legacy':'outer';document.getElementById('scope').textContent=view==='outer'?'Completed placement + whole-board routing evaluations for the latest policy. Earlier fine-grained routing is available in the legacy replay.':'Legacy fine-grained routing history, including historical placement trials; these are not outer placement steps.';if(view==='legacy'){document.getElementById('switch').href='?view=outer';document.getElementById('switch').textContent='Whole-board placement replay';}
