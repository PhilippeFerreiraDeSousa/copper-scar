...
def publish(output,key_file):
    bundle=read(output/'bundle.json');assert bundle['comparison_kind']=='synthetic_topology_search'
    ...
    import wandb
    import requests
    entity='philippe-fdesousa';project='copper-scar';run_id='topology-search-'+bundle['result_sha256'][:12]
    ...
    for row in bundle['rows']:
        ...
        if not previous or not previous.get('ended_at'):
            ...
            trace(f'v2/{entity}/{project}/calls/complete',{'batch':[{'project_id':f'{entity}/{project}','id':call_id,
                'op_name':'synthetic.topology_search','display_name':'Synthetic fixture / '+row['stage'],'trace_id':call_id,
                'started_at':now,'ended_at':now,'attributes':{'comparison_kind':'synthetic_topology_search','product_valid':False},
    ...
    if len(seen)<len(bundle['rows']) or not existing.summary.get('proof_upload_complete'):
        sdk=output/'sdk';sdk.mkdir(exist_ok=True)
        run=wandb.init(entity=entity,project=project,id=run_id,resume='allow',name='Synthetic fixture · KRT placement and topology search',
            job_type='synthetic-fixture-proof',group='synthetic-topology-search',tags=['synthetic','fixture','not-pcbgolf-progress'],dir=str(sdk),
        ...
        try:
            ...
            for row,trace_url in zip(bundle['rows'],traces):
                ...
                run.log({'fixture/step':row['step'],'fixture/stage':row['stage'],
                         ...
                         'fixture/realized_nets':row['realized_nets'],'fixture/vias':row['vias'],'fixture/wire_length_mm':row['wire_length_mm'],'fixture/decision':row['decision'],'fixture/build_wall_s':row['build_wall_s'],
                         'fixture/board_sha256':row['board_sha256'],'comparison_kind':'synthetic_topology_search',
            ...
            run.summary.update({'proof_upload_complete':True,'fixture_decision':bundle['decision'],'product_valid':False,
                               'routing_worker_running':False,'fixture_native_build_wall_s':bundle['native_build_wall_s']})
        finally:run.finish()
    api.flush();remote=api.run(run_path);rows=[r for r in remote.scan_history() if 'fixture/stage' in r]
    assert len(rows)==len(bundle['rows'])
    ...
    assert sha(Path(download.name))==sha(output/'media'/bundle['rows'][-1]['image'])
