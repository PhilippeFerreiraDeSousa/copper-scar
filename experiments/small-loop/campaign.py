...
def command(argv,f,label):
 ...
 with (f/(label+'.log')).open('w') as log:r=subprocess.run(list(map(str,argv)),stdout=log,stderr=subprocess.STDOUT,timeout=100)
...
def realize(base,folder,placements,manifest,source,route=True):
 folder.mkdir();shutil.copy2(base/'authoritative-project.json',folder/'pcbgolf.kicad_pro');shutil.copytree(base/'input/pcbgolf.pretty',folder/'pcbgolf.pretty');shutil.copy2(base/'input/fp-lib-table',folder/'fp-lib-table');putposes(base/'input/small-loop.kicad_pcb',folder/'pcbgolf.kicad_pcb',placements)
 project=(folder/'pcbgolf.kicad_pro').read_bytes();cmds=[]
 def native(action):
  cmds.append(command([KIPY,ROOT/'experiments/small-loop/native_stage.py',action,folder],folder,action))
 ...
 cmds.append(command([KICAD,'pcb','drc','--format','json','-o',folder/'preflight.json',folder/'pcbgolf.kicad_pcb'],folder,'preflight'))
 pre=json.loads((folder/'preflight.json').read_text());legal=not pre['violations']
 if route and legal:
  cmds.append(command([sys.executable,ROOT/'scripts/copperhead_route.py',folder,'--seconds','60','--passes','30','--whole-board','--skip-fanout'],folder,'full-route'));native('import')
 native('audit');cmds.append(command([KICAD,'pcb','drc','--format','json','-o',folder/'drc.json',folder/'pcbgolf.kicad_pcb'],folder,'drc'))
