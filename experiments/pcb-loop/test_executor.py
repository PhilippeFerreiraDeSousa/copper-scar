...
@pytest.mark.parametrize('value,qualified',[(None,True),(float('nan'),True),(float('inf'),True),(True,True),(100,False)])
def test_stage_two_rejects_unqualified_baseline_before_backend(tmp_path,value,qualified):
 req=request(tmp_path,2);req['incumbent'].update(official_score=value,official_score_qualified=qualified)
 def forbidden(*args):raise AssertionError('Backend must not run')
 with pytest.raises(ValueError,match='finite qualified incumbent'):
  module.execute_candidate(req,forbidden,tmp_path/'run')
