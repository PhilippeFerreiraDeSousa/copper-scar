...
def evaluate(request,folder,emit):
    ...
    native('export');shutil.copy2(folder/'pcbgolf.kicad_pcb',folder/'preview.kicad_pcb');run([KICAD,'pcb','drc','--schematic-parity','--format','json','-o',folder/'preflight.json',folder/'pcbgolf.kicad_pcb'],'preflight');pre=json.loads((folder/'preflight.json').read_text());physical=assembly_clearance(folder);write(folder/'preflight-assembly.json',physical);legal=not pre['violations'] and not pre.get('schematic_parity',[{}]) and physical['ok']
    if legal:
