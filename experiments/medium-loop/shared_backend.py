...
def evaluate(request,folder,emit):
    ...
    native('export');dsn=sx.loads((folder/'pcbgolf.dsn').read_text().replace('(string_quote ")',''));structure=first(dsn,'structure');dsn_layers=[str(v[1]) for v in nodes(structure,'layer')];via_names=[str(v) for v in first(structure,'via')[1:]];via_sizes=[list(map(lambda x:float(x)/1000,re.search(r'_(\d+):(\d+)_um$',v).groups())) for v in via_names];scope={'layers':dsn_layers,'via_options':via_sizes,'passed':dsn_layers==request['eligible_layers'] and sorted(via_sizes)==sorted(request['via_options'])};write(folder/'dsn-scope.json',scope);shutil.copy2(folder/'pcbgolf.kicad_pcb',folder/'preview.kicad_pcb');run([KICAD,'pcb','drc','--schematic-parity','--format','json','-o',folder/'preflight.json',folder/'pcbgolf.kicad_pcb'],'preflight');pre=json.loads((folder/'preflight.json').read_text());physical=assembly_clearance(folder);write(folder/'preflight-assembly.json',physical);seed_ids={str(first(v,'uuid')[1]) for v in nodes(board,'via')} if request.get('preserve_seed_copper') is True else set();pending_seed=[v for v in pre['violations'] if v['type']=='via_dangling' and v.get('items') and all(i['uuid'] in seed_ids for i in v['items'])];blocking=[v for v in pre['violations'] if v not in pending_seed];write(folder/'preflight-seed-classification.json',{'intentional_pending_seed_connections':pending_seed,'blocking_violations':blocking,'final_dangling_checks_unchanged':True});legal=scope['passed'] and not blocking and not pre.get('schematic_parity',[{}]) and physical['ok']
    if legal:
