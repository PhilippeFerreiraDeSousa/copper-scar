...
def proposal(template,factor,margin=1):
 ...
 for f in nodes(d,'footprint'):
  r=next(v[2] for v in nodes(f,'property') if v[1]=='Reference');at=first(f,'at');old=at[1:];x=round(55+(old[0]-55)*factor,4);y=round(50+(old[1]-50)*factor,4);at[1:]=[x,y,*old[2:]];poses[r]={'before':old,'after':at[1:]}
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('base',type=Path);ap.add_argument('--source',type=Path,required=True);ap.add_argument('--study',default='');ap.add_argument('--margin',type=float,default=1);ap.add_argument('--factors',default='1,.8,.65,.5,.4');ap.add_argument('--incumbent',type=Path);ap.add_argument('--geometry',type=Path);a=ap.parse_args();base=a.base.resolve();root=base/'stage2';source=a.source.resolve();baseline=root/'baseline';root=root/a.study if a.study else root;root.mkdir(exist_ok=True);manifest=base/'input/circuit.json';commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip();incdir=a.incumbent.resolve() if a.incumbent else baseline;inc=json.loads((incdir/'score.json').read_text());assert inc['valid'];geometry=a.geometry.resolve() if a.geometry else baseline/'pcbgolf.kicad_pcb'
 protocol={'formula':'PCBA bbox volume mm^3 +50*via_count +5000*copper_layers','source':'https://comma.ai/leaderboard','source_sha':commit,'initial_board_sha256':inc['board_sha256'],'baseline_score':inc['official_formula_score'],'objective':'Strict lower official-formula score, after full native/assembly acceptance','factors':[float(x) for x in a.factors.split(',')],'geometry_source':str(geometry),'geometry_sha256':hashlib.sha256(geometry.read_bytes()).hexdigest(),'initial_incumbent':str(incdir),'route_seconds':240,'route_passes':100,'full_board':True,'layers':2,'tie_policy':'retain incumbent','assembly_contract':'assembly-contract.json','wire_length_is_objective':False,'edge_margin_mm':a.margin};write(root/'protocol.json',protocol);events=[]
 for idx,factor in enumerate(protocol['factors'],1):
  ...
  parent_poses=inventory(incdir/'pcbgolf.kicad_pcb')[0];action['geometry_transform_poses']=action['component_poses'];action['component_poses']={r:{'before':parent_poses[r]['pose'],'after':v['after']} for r,v in action['geometry_transform_poses'].items()};dims=action['outline_mm'];expected_volume=(dims[2]-dims[0])*(dims[3]-dims[1])*inc['assembly']['dimensions_mm'][2];action['expected_score_terms']={'qualification':'Forecast only, fixedheight andexistingviacount; nativeassemblyandactualrouting decide score','volume_mm3':expected_volume,'volume_delta_mm3':expected_volume-inc['terms']['pcba_bbox_volume_mm3'],'via_count_assumption':inc['terms']['via_count'],'via_penalty_assumption':inc['terms']['via_penalty'],'layer_penalty':inc['terms']['layer_penalty'],'score_if_vias_unchanged':expected_volume+inc['terms']['via_penalty']+inc['terms']['layer_penalty']};metadata=geometry.parent/'proposal.json';action['source_primitive']=json.loads(metadata.read_text()) if metadata.exists() else {'kind':'uniform_spacing','reason':'Exploratoryvolumecontraction'};write(f/'proposal.json',action);project=(f/'pcbgolf.kicad_pro').read_bytes();cmds=[]
  def native(kind):
