...
The intermediate native result is 54 missing pairs, zero physical errors, 18 warnings and passing invariants. The immutable `fanout-project/` snapshot has aggregate design hash `56043e47761f9e98121555b6d836b776241dd4d70bcd3a6b39eb8840ad03aa77`. Full 600-second routing completed with the same 54 missing pairs, zero physical errors, 18 warnings and passing invariants, and became the retained incumbent. Final board SHA-256 is `785d2ebc0d4d98e781b07c3d04849f7e2469f037c47ebecd4a561f05129901ce`; aggregate design hash is `c60a7ad264723dd66d65b170c197da556f7a1fb58335f787a44e2bc988151e24`. Independent final graph analysis confirms both U15 pads join both original ground planes, only the GND partition changes from 11 to 10 islands, and no connected pad group on any net splits. The initial probe used stock loading defaults; explicit original hole/edge settings were added afterward and are not retroactively claimed for this probe. Native project rules and acceptance checks were unchanged throughout.
...
## KRT producer-rule preservation

A separate exact-parent CAN0_H KRT probe produced no geometry or connectivity gain, but its output project lowered hole clearance from 0.25 to 0.2 mm despite `--no-fix-drc-settings`. The pinned source in-run floor synchronization bypasses that flag. The adapter already copied only the producer board into a candidate with original support files. It now additionally disables that supported synchronization knob (`KICAD_INRUN_FLOOR_SYNC=0`), records producer project semantic differences as a rule-preservation violation, and explicitly restores original candidate project bytes before independent grading. Producer artifacts remain intact for inspection.
