...
def main():
 ...
 for (policy,scope),items in groups.items():
  ...
  for index,(path,r) in enumerate(items):
   ...
   if not found:
    call=client.create_call('copperhead.outer.native_evaluation_backfill',inputs={'attempt_id':r['attempt'],'action':r.get('action'),'placement':r.get('placement_delta'),'fanout':r.get('fanout_result'),'fanout_evaluation':r.get('fanout_evaluation'),'via_seed':r.get('via_seed'),'via_seed_evaluation':r.get('via_seed_evaluation'),'selection_decision':r.get('selection_decision'),'routing_options':r.get('routing_options'),'realization_context':r.get('realization_context'),'effective_routing_options':r.get('effective_routing_options'),'terminal_scope_correction':json.loads((path.parent/'terminal-scope-correction.json').read_text()) if (path.parent/'terminal-scope-correction.json').exists() else None,'routing_scope':route,'routing_coverage':coverage,'baseline_hash':baseline,'evidence_sha256':hashlib.sha256(path.read_bytes()).hexdigest()},attributes={'track':'copperhead','policy':policy,'comparison_kind':r['comparison_kind'],'wb_run_id':run_id,'historical_backfill':True},display_name='Copperhead outer '+str(index)+' '+r['attempt'],_call_id_override=call_id,started_at=datetime.fromisoformat(r['started_at']))
  ...
  for path,record in items:
   ...
   item={**audit,'effective_retained':audit.get('effective_retained',False),'historical_retained':record.get('became_incumbent'),'correction_sha256':hashlib.sha256(correction_path.read_bytes()).hexdigest(),'call_id':call_id}
   if not list(client.get_calls(filter={'call_ids':[call_id]},limit=1)):
