...
def inspect(path):
    ...
    for item in board.GetTracks():
        if isinstance(item,p.PCB_VIA):
            ...
            rows[-1]['diameters_by_layer_nm']={board.GetLayerName(layer):item.GetWidth(layer) for layer in board.GetEnabledLayers().Seq() if p.IsCopperLayer(layer) and item.IsOnLayer(layer)}
    return rows


def key(row):
    return tuple(row[x] for x in ('net','x_nm','y_nm','diameter_nm','drill_nm'))+tuple(row['layers'])+(tuple(sorted(row['diameters_by_layer_nm'].items())),)
