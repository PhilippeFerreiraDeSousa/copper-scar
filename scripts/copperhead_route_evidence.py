...
def evidence(folder):
 ...
 log_path=folder/'router.log';logs=log_path.read_text() if log_path.exists() else '';execution=json.loads((folder/'execution.json').read_text()) if (folder/'execution.json').exists() else {}
 ...
 if not log_path.exists() and not execution:
  result['termination']='not_started'
  result['qualification']='DSN export exists but no whole-board router execution evidence; an earlier action may have failed'
 return result
