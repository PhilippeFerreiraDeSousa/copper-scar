"""Realize a bounded explicit through-via hypothesis; generate no trace path."""
...
seeds=cfg['via_sites'];assert 1<=len(seeds)<=8,'One bounded compatible via-site hypothesis per trial'
assert {site['net'] for site in seeds}==set(cfg['nets'])
...
for site in seeds:
 assert site['net'] in cfg['nets'] and site['diameter_mm']==.45 and site['drill_mm']==.2 and site['span']==['F.Cu','B.Cu'] and site.get('free',True)
...
assert original<={item.m_Uuid.AsString() for item in board.GetTracks()} and len(board.GetTracks())==len(original)+len(seeds)
