...
for site in seeds:
 ...
 via=p.PCB_VIA(board);via.SetViaType(p.VIATYPE_THROUGH);via.SetLayerPair(p.F_Cu,p.B_Cu);via.SetWidth(p.FromMM(.45));via.SetDrill(p.FromMM(.2));via.SetPosition(p.VECTOR2I(p.FromMM(site['position_mm'][0]),p.FromMM(site['position_mm'][1])));via.SetNetCode(nets[site['net']].GetNetCode());board.Add(via)
...
result=dict(parent_board_sha256=cfg['parent_board_sha256'],created_vias=created,generated_tracks=0,qualification='Explicit site only; independent native preflight and full autorouting required. No connection or route feasibility inferred from site legality.')
