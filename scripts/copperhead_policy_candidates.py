"""Bind the shared finite proposal library to a current native arm incumbent."""
...
ref='R123';f=fs[ref];nets=sorted({q.GetNetname() for q in f.Pads() if q.GetNetname()});position=f.GetPosition()
def proxy(dx,dy):
 total=0
 for v in evaluation['violations']:
  items=v.get('items',[])
  if v['type']!='unconnected_items' or len(items)!=2 or not any(' of R123 on ' in i.get('description','') for i in items):continue
  points=[]
  for i in items:
   pt=i['pos'];moved=' of R123 on ' in i.get('description','');points.append((pt['x']+(dx if moved else 0),pt['y']+(dy if moved else 0)))
  total+=math.dist(*points)
 return total
for ident,dx,dy in [('r123-left-two',-2,.5),('r123-left-one',-1,0),('r123-left-half',-.5,0)]:
 actions.append(dict(id=ident,kind='group_pose',parent_board_sha256=sha,refs=[ref],locked_refs=sorted(set(fs)-{ref}),from_poses={ref:dict(x_mm=p.ToMM(position.x),y_mm=p.ToMM(position.y),angle_deg=f.GetOrientationDegrees())},translation_mm=[dx,dy],rotation_deg=0,nets=nets,net=nets[0],group='port_channel_1',group_members=[ref],copper_policy='detach_moved_pad_incident',boundary_via_policy='preserve_existing_sites',preserve_foreign_copper=True,proxy_before=dict(total=proxy(0,0)),proxy_after=dict(total=proxy(dx,dy)),local_approach_mm=proxy(dx,dy),reason='Shared finite native-bound R123 placement library',hypothesis='Shorter disconnected endpoint approach may improve full-board realizability',feedback_used=[str(a.evaluation)]))
