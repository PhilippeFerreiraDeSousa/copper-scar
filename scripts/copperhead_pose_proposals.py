...
ap=argparse.ArgumentParser();ap.add_argument('parent',type=Path);ap.add_argument('--manifest',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();started=time.monotonic()
...
# Choose the known SD interface as one bounded test; all other bodies are fixed.
group='sd_card_interface';refs=groups[group];ignored={i for i,n in data.nets.items() if n.name in ('GND','+3V3','+5V','+12V')}
...
deltas=[(x,0) for x in (-15,-10,-5,5,10,15)]+[(0,y) for y in (-15,-10,-5,5,10,15)]+[(x,y) for x,y in [(-15,-15),(-15,15),(15,-15),(15,15),(-10,-10),(-10,10),(10,-10),(10,10),(-5,-5),(-5,5),(5,-5),(5,5)]]
trials=[]
for dx,dy in deltas:
...
result={'schema_version':1,'kind':'group_pose','parent_candidate':str(a.parent.resolve()),'parent_board_sha256':before_hash,'group':group,'refs':refs,'groups':groups,'locked_refs':sorted(set(flat)-set(refs)),'from_poses':poses,'translation_mm':selected[0]['translation_mm'],'nets':affected,'net':'Net-(J2-CMD)','reason':'Research-guided rigid SD neighborhood search on expanded baseline; preserve tight electrical group geometry and unrelated poses','hypothesis':'Improve relative interface placement while keeping outline and within-group poses fixed; geometric proxy does not establish routing gain','feedback_used':['stage1-20260912-185217-d06b3b','pcb-placement-research.md'],'proxy_before':baseline,'proxy_after':selected[0]['proxy'],'proxy_ignored_nets':['GND','+3V3','+5V','+12V'],'full_board_evaluation_includes_ignored_proxy_nets':True,'trials':trials,'finalists':selected,'diagnosis':diagnosis,'krt_groups_derived':derived,'krt_revision':'1c428c0b2285a4dfe8901ca7035109ded6cfbb4d','screen_limitation':'KRT ranking/geometry only, pad legality layer off; native physical/invariant recheck mandatory, retained copper not modeled by KRT','elapsed_seconds':time.monotonic()-started}
...
assert result['proxy_after']['total']<baseline['total'],'No cheaper proposal; do not fake progress'
