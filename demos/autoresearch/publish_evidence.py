...
def main():
 ...
 import wandb,weave
 api=wandb.Api();client=weave.init(PROJECT);jobs=[]
 if a.historical:
  ...
  for policy in state['policies']:
   ...
   for point in policy['points']:
    ...
    files={'attempt.json':out/point['receipt_href']}
 ...
 for rid,attempt,files,parentid in jobs:
  ...
  parent=list(client.get_calls(filter={'call_ids':[parentid]},limit=1));assert parent,'Missing parent trace';cid=stable('complete-evidence/'+rid+'/'+bundle_hash);calls=list(client.get_calls(filter={'call_ids':[cid]},limit=1));links={'artifact':qualified,'run_files':'https://wandb.ai/'+PROJECT+'/runs/'+rid+'/files','run_artifacts':'https://wandb.ai/'+PROJECT+'/runs/'+rid+'/artifacts','download_note':'Download complete-native-evidence artifact for full untruncated JSON and the listed exact CAD files. Run Files also contains attempt.json.','files_sha256':hashes}
  if not calls:
   c=client.create_call('copperhead.complete_evidence',{'attempt':attempt,'full_record_sha256':hashes['attempt.json'],'truncated_display_is_not_full_record':True},parent=parent[0],display_name='Download complete original record',use_stack=False,_call_id_override=cid);client.finish_call(c,output=links)
  found=list(client.get_calls(filter={'call_ids':[cid]},limit=1));assert found and found[0].ended_at
