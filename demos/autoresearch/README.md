...
- `watch.py` with the same arguments runs the builder, idempotent event publisher, and labeled router-log snapshots in one foreground process. It continues watching separate next-campaign receipts; `--stop-after-decision` ends after the recorded policy decision. Use the protected owner observability Python environment for W&B/Weave.
...
- `publish_evidence.py` publishes complete JSON/CAD/native proof bundles, adds readable evidence summaries and verifies downloaded SHA256 bytes. `--historical` binds the original 073500 record.
- `trace_io.py` uses the project-compatible synchronous v2 trace lifecycle API with bounded exact-output readback; it preserves existing span IDs.
- `verify.py` checks offline rendering across all six copper layers, receipt hashes, replay chronology and mobile overflow.
- `package.py --outputs DIR` streams a pilot-first ZIP with exact source Git archives, original input/support, evidence, replay and demo material. It verifies every archived file. `--allow-incomplete` explicitly labels a partial package.
