...
def main():
 ...
 for path,d in entries:
  ...
  receipt={k:d[k] for k in ['attempt','policy','started_at','finished_at','status','input','candidate','action','action_level','comparison_kind','incumbent_before','incumbent_after','became_incumbent','constraint_scope','routing_scope','classification','selection_decision','diagnostic_improved','tool_source_hashes'] if k in d}
  ...
  inc=(correction['restored'] if correction else d.get('incumbent_after')) or {}; priority=inc.get('priority') or []
 ...
 if not (out/'publication-data.js').exists(): (out/'publication-data.js').write_text('window.PUBLICATION=null;\n')
 if not (out/'seed-data.js').exists(): (out/'seed-data.js').write_text('window.SEED=null;\n')
