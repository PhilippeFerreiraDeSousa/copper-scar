...
| 1:20–1:50 | Rejected outer candidates, diagnostic details | “Now look at a failure. We expanded component groups and routed under a fixed effort limit. A completed trial still had 477 opens, so it did not replace the incumbent. Moving the SD group produced an exploratory 472-open result. The changed opens were on other nets; we cannot claim the SD move caused an improvement. That diagnostic is useful: it prevents the next proposal from learning the wrong lesson.” |
