...
publication=json.loads((out/'publication/summary.json').read_text()) if (out/'publication/summary.json').exists() else None
commit=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip()
(out/'SOURCE-CHECKPOINT.json').write_text(json.dumps(dict(repository='https://github.com/PhilippeFerreiraDeSousa/copper-scar',branch='codex/offline-demo',commit=commit,evidence_built_at=d['built_at'],latest_completed_attempt=latest['id'],retained_attempt=retained['id'],retained_board_sha256=retained['board_sha256'],published_board_sha256=publication['board_sha256'] if publication else None,publication_transformation=publication['transformation'] if publication else None,additive_diagnostic_packaged_at=diagnostic['packaged_at'] if diagnostic else None,source_scope='Offline demo source only; engines retain separately recorded tool source hashes.'),indent=2)+'\n')
...
if publication:
 with (out/'METRICS-AS-OF.md').open('a') as f:f.write(f"\nHistorical source `{publication['source_board_sha256']}` retains original evidence/receipts and stale stored fills. Published CAD `{publication['board_sha256']}` is a separate private refill/save with authored CAD preserved, fresh saved-file DRC45/0/18, and a STEP regenerated from that exact published hash. See publication/ and assembly/. This is not a new optimization attempt or W&B result.\n")
if diagnostic:
 with (out/'METRICS-AS-OF.md').open('a') as f:f.write(f"\nAdditive native diagnostic packaged {diagnostic['packaged_at']}, bound to parent {diagnostic['parent_board_sha256']}: nine nets, 32 of55pairs. This parent-specific diagnostic is separate from the current selected state. Video scope and timing are recorded in video-manifest.json.\n")
