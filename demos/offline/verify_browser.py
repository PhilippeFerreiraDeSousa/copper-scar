...
with sync_playwright() as p:
 ...
 confirmed=[(i,r) for i,r in enumerate(data['history']) if r.get('selection_confirmation')]
 for i,r in confirmed:
  page.evaluate('(i)=>showAttempt(i)',i);assert page.locator('#retained').inner_text()=='RETAINED · SELECTION BASIS CONFIRMED'
  assert page.locator('#correction').inner_text()=='Selection confirmation ↗' and page.locator('#correction').get_attribute('href')==r['selection_confirmation']
  assert page.locator('#original-audit').is_visible()
